...
#def run_servo_stepper(Servo_P_gain, Servo_I_gain, Servo_D_gain, Servo_I_windup, Servo_Bias, Servo_Min, Servo_Max, Servo_t_pid = 0.0, Servo_Derivator = 0.0, Servo_Integrator = 0.0, heading = 0.0, stepper_error = 0.0, stepper_error1 = 0.0, speed = 0.0,  stepper_direction = 0.0, t_print = 0.0):
while True:
    ...
    # getting setpoints from gui/config modules
    heading_sp =config.heading_sp
    Servo_pitch_sp = config.pitch_sp
    ...
    if (t_interval >= t_print + 1):
        t_print = t_interval
        print("Tracker Heading is ", heading, "heading sp is ", heading_sp, " error is ",  stepper_error, "error1 is ", stepper_error1, "Speed set to ", speed, "Direction set to ", stepper_direction)        
...
if __name__ == "__main__":
    run_servo_stepper()    
    print( "Tracker module started locally")
else:
    print("Tracker module started remotely")
'''
